...
let opts ?draw_points ?show_roller ?value_range ?file ?legend ?labels ?title ?ylabel ?connect_separated_points () =
  let o = Jv.obj [||] in
  ...
  Option.iter (fun (lo, hi) -> Jv.set o "valueRange" (Jv.of_list Jv.of_float [lo; hi])) value_range;
