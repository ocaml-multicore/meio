  ...
  | length -> (
      ...
      | false ->
          ...
          while (not !added_new_entry) && !idx < length do
            if Jv.to_float dom_acc.:[!idx].:[0] = ts then (
              ...
              dom_acc.:[!idx].:[domain_id + 1] <-
                Jv.of_float
                  (Jv.to_float dom_acc.:[!idx - 1].:[domain_id + 1] +. v);
              update_after_existing ~dom_acc ~domain_id !idx;
              added_new_entry := true)
            else if
              !idx > 0
              && ts > Jv.to_float dom_acc.:[!idx].:[0]
              && ts < Jv.to_float dom_acc.:[!idx + 1].:[0]
            then (
