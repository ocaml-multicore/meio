...
let graph =
  let opts =
    Dygraph.opts ~draw_points:true ~show_roller:true
      ~labels:[ "ts"; "dom0"; "dom1" ] ~title:"Requested Major Allocations"
      ~legend:`Always ~ylabel:"words" ~connect_separated_points:true ()
  in
...
let connect () =
  let on_message msg =
    ...
    | Events.System (`Counter ({ name; ts; domain_id }, value)) ->
        ...
        if name = "C_REQUEST_MAJOR_ALLOC_SHR" then (
